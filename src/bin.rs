...
impl<'a> SDLWindow<'a> {
    fn new(
        ...
        if !full_screen {
            ...
            copy_area =
                Some(sdl2::rect::Rect::new(0, 8, PIX_WIDTH, PIX_HEIGHT - 16));
        }
        let window = video_subsystem
            .window("RuNES", actual_width, actual_height)
            .position_centered()
            .opengl()
            .build()
            .unwrap();
...
fn main() {
    ...
    let scale = std::cmp::min(
        8,
        std::cmp::max(1, value_t!(matches, "scale", u32).unwrap_or(3)),
    );
    ...
    let audio_dev = audio_subsystem
        .open_playback(None, &desired_spec, |_| SDLAudioPlayback(&audio_sync))
        .unwrap();
    ...
    let load_state = !no_state &&
        match match load_state_name {
    ...
    loop {
        ...
        if event.is_exiting() {
            {
                let mut file = FileIO(
                    File::create(match save_state_name {
                        Some(s) => s.to_string(),
            ...
            {
                let mut file = FileIO(
                    File::create(match save_sram_name {
                        Some(s) => s.to_string(),
